...
export class AuthService {
     ...
     */
    async createTokens(userId, context, familyId) {
     ...
     */
    async issueOtp(identifier, purpose) {
     ...
     */
    async oauthCallback(provider, profile, context) {
        // 1. Check if we already have this OAuth account
        const existingAccount = await this.repository.findAccountByProvider(provider, profile.id);
        let user;
        if (existingAccount) {
    ...
    async verifyAndEnableMfa(userId, code) {
        ...
        const hashedCodes = await Promise.all(rawRecoveryCodes.map(c => argon2.hash(c, {
            type: argon2.argon2id,
            memoryCost: 65536,
            timeCost: 3,
            parallelism: 4,
     ...
     */
    async requestMfaRecoveryOtp(dto) {
        const userId = verifyMfaToken(dto.mfaToken);
        const user = await this.repository.findUserById(userId);
     ...
     */
    async verifyMfaRecoveryOtp(dto, context) {
        const userId = verifyMfaToken(dto.mfaToken);
        const user = await this.repository.findUserById(userId);
        if (!user || !user.mfaEnabled) {
            throw new AppError("MFA is not enabled for this user", HTTP_STATUS.BAD_REQUEST);
    ...
    async disableMfa(userId, dto) {
        ...
        if (dto.code) {
            ...
            if (!localAccount || !localAccount.password) {
                throw new AppError("Password verification is not available for this account. Please use MFA code.", HTTP_STATUS.BAD_REQUEST);
            }
