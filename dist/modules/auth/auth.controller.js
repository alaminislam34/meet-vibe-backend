...
export const verifyOtp = async (req, res, next) => {
    try {
        ...
        const { user, accessToken, refreshToken } = await authService.verifyOtp({ email, otp }, getRequestContext(req));
...
export const refresh = async (req, res, next) => {
    try {
        ...
        const { accessToken, refreshToken } = await authService.refreshSession(token, getRequestContext(req));
        setAuthCookies(res, accessToken, refreshToken);
        res.status(HTTP_STATUS.OK).json({
            status: "success",
...
export const googleCallback = async (req, res, next) => {
    try {
        ...
        const { user, accessToken, refreshToken } = await authService.oauthCallback("GOOGLE", mockProfile, getRequestContext(req));
        setAuthCookies(res, accessToken, refreshToken);
...
export const appleCallback = async (req, res, next) => {
    try {
        ...
        const { user, accessToken, refreshToken } = await authService.oauthCallback("APPLE", mockProfile, getRequestContext(req));
        setAuthCookies(res, accessToken, refreshToken);
        res.render("oauth-callback", {
            status: "success",
            message: "OAuth login completed",
...
export const mfaLogin = async (req, res, next) => {
    try {
        const { mfaToken, mfaCode } = req.body;
        const { user, accessToken, refreshToken } = await authService.mfaLogin({ mfaToken, mfaCode }, getRequestContext(req));
        setAuthCookies(res, accessToken, refreshToken);
        res.status(HTTP_STATUS.OK).json({
            status: "success",
