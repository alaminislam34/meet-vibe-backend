...
const requireActiveSubscription = async (userId) => {
    const sub = await prisma.subscription.findUnique({ where: { userId } });
    if (!sub || sub.status !== SUB_STATUS.ACTIVE || new Date(sub.endDate) < new Date()) {
        throw new AppError("An active subscription is required to create events.", HTTP_STATUS.FORBIDDEN);
    }
};
// ─── Guard: Event owner check ─────────────────────────────────────────────────
const requireEventOwner = async (eventId, userId) => {
    ...
    if (event.status !== EVENT_STATUS.DRAFT)
        throw new AppError("Event already published or cancelled", HTTP_STATUS.BAD_REQUEST);
    return event;
};
// ─── Step 1 – Basic Info ──────────────────────────────────────────────────────
...
export const publishEvent = async (req, res, next) => {
    try {
        ...
        if (event.creationStep < 4) {
            throw new AppError("Please complete all 4 steps before publishing.", HTTP_STATUS.BAD_REQUEST);
        }
        if (!event.title || !event.startDate || !event.venueType) {
            throw new AppError("Event is missing required fields. Please review all steps.", HTTP_STATUS.UNPROCESSABLE_ENTITY);
        }
