...
export const createEventCheckoutSession = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { eventId } = req.body;
        const event = await prisma.event.findUnique({
        ...
        const session = await createCheckoutSessionForEvent(customer.id, Number(event.price), event.id, user.id, successUrl, cancelUrl);
...
export const finalizeEventAttendance = async (req, res, next) => {
    try {
        ...
        if (!event.isFree && Number(event.price) > 0 && !host.stripeConnectedAccountId) {
            throw new AppError("Host must connect their Stripe Account first to release paid event escrow.", HTTP_STATUS.BAD_REQUEST);
        }
        const results = [];
                ...
                try {
                    if (attended) {
                        if (event.isDepositModel) {
                            ...
                            await createTransferToConnectedAccount(amountInCents, host.stripeConnectedAccountId, `event_${event.id}`);
                        ...
                        if (event.isDepositModel) {
                            ...
                            if (penaltyAmount > 0) {
                                await createTransferToConnectedAccount(penaltyAmount, host.stripeConnectedAccountId, `event_${event.id}`);
                            }
                            paymentStatus = "REFUNDED";
                        }
                        else {
