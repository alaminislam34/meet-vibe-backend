...
export const sendConnectionRequest = async (
  ...
  try {
    ...
    if (!sharedEvent) {
      throw new AppError(
        "You can only connect with users you have shared an event with.",
...
export const respondToConnection = async (
  ...
  try {
    ...
    if (!connection) throw new AppError("Connection request not found.", HTTP_STATUS.NOT_FOUND);
    if (connection.receiverId !== userId) throw new AppError("Unauthorized.", HTTP_STATUS.FORBIDDEN);
...
export const removeConnection = async (
  ...
  try {
    ...
    if (!connection) throw new AppError("Connection not found.", HTTP_STATUS.NOT_FOUND);
