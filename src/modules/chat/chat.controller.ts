...
export const getGroupMessages = async (
  ...
  try {
    ...
    if (!event.chatGroup) throw new AppError("Group chat not found.", HTTP_STATUS.NOT_FOUND);

    const hasAccess = event.creatorId === userId || event.participants.length > 0;
    if (!hasAccess) throw new AppError("Access denied to this group chat.", HTTP_STATUS.FORBIDDEN);
...
export const sendGroupMessage = async (
  ...
  try {
    ...
    if (!message?.trim()) throw new AppError("Message cannot be empty.", HTTP_STATUS.BAD_REQUEST);
    ...
    if (!event || !event.chatGroup) throw new AppError("Event or chat not found.", HTTP_STATUS.NOT_FOUND);

    const hasAccess = event.creatorId === userId || event.participants.length > 0;
    if (!hasAccess) throw new AppError("Access denied.", HTTP_STATUS.FORBIDDEN);
